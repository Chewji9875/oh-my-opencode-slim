...
| `backgroundJobs.waitForUserGuard` | boolean | `true` | When true, intercepts `wait_for_user` calls while background tasks are still outstanding, returning guidance to end the turn instead of blocking on manual input. See [Background Job Management](#background-job-management). |
